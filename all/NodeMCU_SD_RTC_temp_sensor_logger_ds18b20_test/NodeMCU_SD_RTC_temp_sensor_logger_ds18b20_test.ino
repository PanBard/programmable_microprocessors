...
//-------------------SETUP (start)
int time_for_log_save = 10; // interval for log save (in seconds)
//-------------------SETUP (end)
...
bool checkIfIsTimeToMakeLog()
{
  ...
  {
    Serial.println("Enter in IF STATEMENT ----------------------------------------------------------");
    return true;
  }
  else
  {
...
{
    time_from_RTC = Rtc.GetDateTime();
    Serial.println(time_from_RTC.Second());
    Serial.println(time_from_RTC.TotalSeconds());
